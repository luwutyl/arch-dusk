...
static char *colors[SchemeLast][ColCount] = {
	...
	[SchemeNorm]          = { "#D0D7DE", "#1E2228", "norm" },

	/* Фокус / выбранное — яркий синий */
	[SchemeSel]           = { "#FFFFFF", "#3478B5", "sel" },

	/* Вывод / out — холодный акцент, но не режущий глаз */
	[SchemeOut]           = { "#0B1C26", "#4FA3E3", "out" },

	/* Бордер — подчёркивает фокус */
	[SchemeBorder]        = { "#4FA3E3", "#3478B5", "border" },

	/* Prompt — как sel, чтобы фокус ощущался едино */
	[SchemePrompt]        = { "#FFFFFF", "#3478B5", "prompt" },

	/* Adjacent — оставляем красный, чуть приглушённый */
	[SchemeAdjacent]      = { "#FFFFFF", "#8A2E2E", "adjacent" },

	/* Подсветка в фокусе */
	[SchemeSelHighlight]  = { "#FFD58A", "#3478B5", "selhl" },

	/* Подсветка без фокуса */
	[SchemeNormHighlight] = { "#FFC978", "#1E2228", "normhl" },

	/* Hp — чуть светлее нормы */
	[SchemeHp]            = { "#C9D1D9", "#2A2F36", "hp" },
};
